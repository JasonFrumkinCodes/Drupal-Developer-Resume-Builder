...
class SkillsBlock extends BlockBase {
  ...
  public function build() {
    ...
    $skills = array();
    $output = "<p class='developer_resume_title'>".$this->configuration['block_title']."</p>";
    $output .= "<ul>";
    foreach($skills_taxonomy as $skills_taxonomy_term) {
        $weight = \Drupal::entityTypeManager()->getStorage('taxonomy_term')->load($skills_taxonomy_term->tid)->get('display_weight')->getValue()[0]['value'];
